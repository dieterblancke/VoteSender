...
public class VoteSendCommand extends Command {
    ...
    public boolean execute(CommandSender sender, String aliase, String[] args) {
        if (!sender.hasPermission("votesend.admin")) {
            sender.sendMessage("&c&lVoteSender &fYou don't have the permission to do this!");
            return false;
        }
        if (args.length == 1) {
            if (args[0].contains("reload") || args[0].contains("rl")) {
                VoteSender.getInstance().reload();
                sender.sendMessage("&c&lVoteSender &fThe config has been reloaded!");
                return true;
            }
            ...
            sender.sendMessage("&c&lVoteSender &fYou have sent a test vote for &c" + player + "&f!");
            return true;
        }
        sender.sendMessage("&c&lVoteSender &fHelp:");
        sender.sendMessage("&c- &f/votesend reload &7| Reloads the plugin.");
        sender.sendMessage("&c- &f/votesend send (player) (service) &7| Sends a test vote.");
        sender.sendMessage("&c- &f/votesend help &7| Returns this list.");
        sender.sendMessage("&c- &f/votesend amount &7| Returns the amount of votes sent through.");
        return false;
    }
}